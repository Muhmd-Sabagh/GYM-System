                    ...
                    foreach (var row in initialAssessmentData.Skip(1)) // Skip header row
                    {
                        try
                        {
                            ...
                            var assessment = new ClientAssessment
                            {
                                ...
                                // Client Information (Questions 3-10)
                                Country = row.ElementAtOrDefault(3)?.ToString() ?? string.Empty,
                                Religion = row.ElementAtOrDefault(4)?.ToString() ?? string.Empty,
                                ServiceGoal = row.ElementAtOrDefault(5)?.ToString() ?? string.Empty,
                                WeightKg = ParseDecimal(row.ElementAtOrDefault(6)) ?? 0,
                                HeightCm = ParseDecimal(row.ElementAtOrDefault(7)) ?? 0,
                                Gender = row.ElementAtOrDefault(8)?.ToString() ?? string.Empty,
                                DateOfBirth = DateTime.TryParseExact(row.ElementAtOrDefault(9)?.ToString(), "M/d/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out var dob) ? dob : DateTime.MinValue,
                                JobProfession = row.ElementAtOrDefault(10)?.ToString() ?? string.Empty,

                                // Body Assessment (Questions 11-18)
                                NeckCircumferenceCm = ParseDecimal(row.ElementAtOrDefault(11)),
                                WaistCircumferenceCm = ParseDecimal(row.ElementAtOrDefault(12)),
                                HipCircumferenceCm = ParseDecimal(row.ElementAtOrDefault(13)),
                                ArmCircumferenceCm = ParseDecimal(row.ElementAtOrDefault(14)),
                                ThighCircumferenceCm = ParseDecimal(row.ElementAtOrDefault(15)),
                                FrontBodyPhotoPath = row.ElementAtOrDefault(16)?.ToString(),
                                SideBodyPhotoPath = row.ElementAtOrDefault(17)?.ToString(),
                                BackBodyPhotoPath = row.ElementAtOrDefault(18)?.ToString(),

                                // Medical Assessment (Questions 19-33)
                                HasHealthProblems = ParseBool(row.ElementAtOrDefault(19)),
                                HealthProblemsDetails = row.ElementAtOrDefault(20)?.ToString(),
                                HasRecentTests = ParseBool(row.ElementAtOrDefault(21)),
                                RecentTestsDetails = row.ElementAtOrDefault(22)?.ToString(),
                                IsTakingMedicationsSupplements = ParseBool(row.ElementAtOrDefault(23)),
                                MedicationsSupplementsDetails = row.ElementAtOrDefault(24)?.ToString(),
                                HasMedicationAllergies = ParseBool(row.ElementAtOrDefault(25)),
                                MedicationAllergiesDetails = row.ElementAtOrDefault(26)?.ToString(),
                                HasChronicHereditaryDiseases = ParseBool(row.ElementAtOrDefault(27)),
                                ChronicHereditaryDiseasesDetails = row.ElementAtOrDefault(28)?.ToString(),
                                HasPastSurgeries = ParseBool(row.ElementAtOrDefault(29)),
                                PastSurgeriesDetails = row.ElementAtOrDefault(30)?.ToString(),
                                HasInjuries = ParseBool(row.ElementAtOrDefault(31)),
                                InjuriesDetails = row.ElementAtOrDefault(32)?.ToString(),
                                IsSmoker = ParseBool(row.ElementAtOrDefault(33)),

                                // Dietary Assessment (Questions 34-52)
                                HasPreviousDietCommitment = ParseBool(row.ElementAtOrDefault(34)),
                                PreviousDietExperience = row.ElementAtOrDefault(35)?.ToString(),
                                DailyEffortDescription = row.ElementAtOrDefault(36)?.ToString() ?? string.Empty,
                                DietCommitmentObstacles = row.ElementAtOrDefault(37)?.ToString() ?? string.Empty,
                                DrinksSpecificBeverages = ParseBool(row.ElementAtOrDefault(38)),
                                BeverageConsumptionDetails = row.ElementAtOrDefault(39)?.ToString(),
                                LastDietSystemAvailable = row.ElementAtOrDefault(40)?.ToString(),
                                HasFoodAllergies = ParseBool(row.ElementAtOrDefault(41)),
                                FoodAllergyDetails = row.ElementAtOrDefault(42)?.ToString(),
                                DislikesSpecificFood = ParseBool(row.ElementAtOrDefault(43)),
                                DislikedFoodDetails = row.ElementAtOrDefault(44)?.ToString(),
                                WantsVitaminsMinerals = ParseBool(row.ElementAtOrDefault(45)),
                                AvailableDesiredVitaminsMinerals = row.ElementAtOrDefault(46)?.ToString(),
                                DesiredMainMealsCount = ParseInt(row.ElementAtOrDefault(47)) ?? 0,
                                DietFlexibilityPreference = row.ElementAtOrDefault(48)?.ToString() ?? string.Empty,
                                DietBudget = row.ElementAtOrDefault(49)?.ToString() ?? string.Empty,
                                PreferredProteinTypes = row.ElementAtOrDefault(50)?.ToString() ?? string.Empty,
                                PreferredCarbohydrateTypes = row.ElementAtOrDefault(51)?.ToString() ?? string.Empty,
                                PreferredHealthyFatTypes = row.ElementAtOrDefault(52)?.ToString() ?? string.Empty,

                                // Workout Assessment (Questions 53-64)
                                WorkoutExperience = row.ElementAtOrDefault(53)?.ToString() ?? string.Empty,
                                ResistanceTrainingDuration = row.ElementAtOrDefault(54)?.ToString() ?? string.Empty,
                                PracticesOtherSports = ParseBool(row.ElementAtOrDefault(55)),
                                OtherSportsDetails = row.ElementAtOrDefault(56)?.ToString(),
                                WorkoutLocation = row.ElementAtOrDefault(57)?.ToString() ?? string.Empty,
                                AvailableHomeEquipment = row.ElementAtOrDefault(58)?.ToString(),
                                AvailableWorkoutDaysCount = ParseInt(row.ElementAtOrDefault(59)) ?? 0,
                                AvailableWorkoutDays = row.ElementAtOrDefault(60)?.ToString() ?? string.Empty,
                                HasExerciseDiscomfort = ParseBool(row.ElementAtOrDefault(61)),
                                DiscomfortExercisesDetails = row.ElementAtOrDefault(62)?.ToString(),
                                PreferredCardioType = row.ElementAtOrDefault(63)?.ToString() ?? string.Empty,
                                DailyStepsCount = row.ElementAtOrDefault(64)?.ToString() ?? string.Empty,

                                // General Assessment (Questions 65-67)
                                PreviousOnlineTrainingExperience = row.ElementAtOrDefault(65)?.ToString(),
                                ReasonForSubscription = row.ElementAtOrDefault(66)?.ToString() ?? string.Empty,
                                OtherNotes = row.ElementAtOrDefault(67)?.ToString()
                            };
                    ...
                    foreach (var row in updateFormData.Skip(1)) // Skip header row
                    {
                        try
                        {
                            ...
                            var update = new ClientUpdate
                            {
                                ...
                                CurrentWeightKg = ParseDecimal(row.ElementAtOrDefault(3)) ?? 0,

                                // Dietary Assessment Update (Questions 3-11)
                                NeckCircumferenceCm = ParseDecimal(row.ElementAtOrDefault(4)),
                                WaistCircumferenceCm = ParseDecimal(row.ElementAtOrDefault(5)),
                                HipCircumferenceCm = ParseDecimal(row.ElementAtOrDefault(6)),
                                ArmCircumferenceCm = ParseDecimal(row.ElementAtOrDefault(7)),
                                ThighCircumferenceCm = ParseDecimal(row.ElementAtOrDefault(8)),
                                FrontBodyPhotoPath = row.ElementAtOrDefault(9)?.ToString(),
                                SideBodyPhotoPath = row.ElementAtOrDefault(10)?.ToString(),
                                BackBodyPhotoPath = row.ElementAtOrDefault(11)?.ToString(),

                                // Body Assessment Update (Questions 12-20)
                                DietCommitmentLevel = row.ElementAtOrDefault(12)?.ToString() ?? string.Empty,
                                HasFoodToRemove = ParseBool(row.ElementAtOrDefault(13)),
                                FoodToRemoveDetails = row.ElementAtOrDefault(14)?.ToString(),
                                HasFoodToAdd = ParseBool(row.ElementAtOrDefault(15)),
                                FoodToAddDetails = row.ElementAtOrDefault(16)?.ToString(),
                                HasFoodToKeepFromPrevious = ParseBool(row.ElementAtOrDefault(17)),
                                FoodToKeepFromPreviousDetails = row.ElementAtOrDefault(18)?.ToString(),
                                DesiredMealsCount = ParseInt(row.ElementAtOrDefault(19)) ?? 0,
                                DietaryNotes = row.ElementAtOrDefault(20)?.ToString(),

                                // Workout Assessment Update (Questions 21-33)
                                WorkoutCommitmentLevel = row.ElementAtOrDefault(21)?.ToString() ?? string.Empty,
                                PreviousWorkoutSystemExperience = row.ElementAtOrDefault(22)?.ToString() ?? string.Empty,
                                HasWeightRepsDevelopment = ParseBool(row.ElementAtOrDefault(23)),
                                IsTrainingVolumeSuitable = ParseBool(row.ElementAtOrDefault(24)),
                                DesiredTrainingVolumeAdjustment = row.ElementAtOrDefault(25)?.ToString(),
                                IsTrainingIntensitySuitable = ParseBool(row.ElementAtOrDefault(26)),
                                DesiredTrainingIntensityAdjustment = row.ElementAtOrDefault(27)?.ToString(),
                                HasExerciseDiscomfort = ParseBool(row.ElementAtOrDefault(28)),
                                DiscomfortExerciseName = row.ElementAtOrDefault(29)?.ToString(),
                                AvailableWorkoutDaysCount = ParseInt(row.ElementAtOrDefault(30)) ?? 0,
                                WorkoutLocation = row.ElementAtOrDefault(31)?.ToString() ?? string.Empty,
                                AvailableHomeEquipment = row.ElementAtOrDefault(32)?.ToString(),
                                Notes = row.ElementAtOrDefault(33)?.ToString()
                            };
